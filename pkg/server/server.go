...
func (s *Server) runIteration(repo repo.Repo, source *model.Source, lastRunTime *time.Time, currentTime time.Time) error {
	...
	s.logger.Info("started to handle source %d by the schedule", source.ID)
	defer s.logger.Info("finished to handle source %d by the schedule", source.ID)

	currCrawler, err := crawlers.NewCrawler(*source, s.logger)
	if err != nil {
		return xerrors.Errorf("unable to create new crawler, err: %w", err)
	}
	currEngine := engine.NewEngine(source, currCrawler, repo)
...
func (s *Server) HandleAllSources(ctx context.Context) error {
	...
	if err != nil {
		return xerrors.Errorf("unable to create tx: %w", err)
	}
	...
	for _, source := range sources {
		err := s.runIteration(currRepo, &source, lastRunTime, currentTime)
		if err != nil {
